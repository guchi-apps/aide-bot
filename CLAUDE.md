...
- **話題の仕入れはアプリを開いたときだけ**（#144）なので、24時間開いていない日は話題が無く黙る。定時の前にcronで仕入れる
  案は、1回約38万トークンと「cronは足さない」を改める必要があり見送った（要判断）
...
なっており、**秘書の立ち絵の全画面（「話す」）はヘッダーの切り替えから今までどおり使える**。
...
- **画面の中央にいるのは秘書の立ち絵**（#326。`src/components/voice/secretary.tsx`）。待つ・聞く・考える・
  話すの4状態と、お知らせを伝える姿を出し分ける（下記「秘書の立ち絵」）
...
**#49のロボット（3Dとその代替のSVG）を、女性秘書の立ち絵5枚に置き換えた。** 絵は
`src/components/voice/secretary.tsx`、動きは `globals.css` の `.sec-*`、素材は `public/secretary/`。

- **状態と絵の対応は `poseFor()` の1か所。** 待機＝基本（`idle`）、聞き取り＝聞く、生成中と
  VOICEVOXの準備中（`preparing`）＝考える、読み上げ中＝話す。**知らせる（`notifying`）は会話の状態では
  ない**——待機中に吹き出しがお知らせ（`BubbleLine.kind === "notice"`）を出している間だけの姿で、
  `VoicePanel` が `notifying` で渡す。ひとりごと・話題のときは基本の絵
- **5枚を重ねて置き、いまの1枚だけを不透明にする**（0.2秒のフェード）。状態が変わるたびに `src` を
  差し替えると、読み込みが間に合わない回に一瞬だけ空白が出る。「動きを減らす」設定では揺れも
  フェードも止め、差し替えだけにする
- **素材は承認済みの原画（1024×1536・単色背景のRGB）から作った。** 外周からの塗りつぶしで背景を抜き、
  髪の隙間・脚の間に閉じ込められた背景色の島も消して（ブラウスは背景色に近いので、胴の帯は対象外）、
  頭〜太もも（`780x860+120+0`）を切り出したWebP（1枚約40KB）。**細い髪の縁にはわずかに白が残る**ので、
  後ろに淡い円（`.sec-stage`）を敷き、腰から下はマスクで消している。原画を差し替えるときは、Issue #326の
  添付から同じ手順で作り直す
- **絵は飾りで、押せる部品にしない**（`aria-hidden`）。ロボットにあった回転（#201）・視線追従（#180）・
  部位タップ（#180・#215）は、平面の絵では成り立たないのでやめた。three.js も依存ごと外してある
- **画像を読み込めなかったら、立ち絵の場所に小さな印（「秘書」）だけを出す。** 会話と操作は絵に
  依存していない。**ハイドレーションより前に失敗した画像は `onError` へ届かない**ので、マウント時に
  `complete && naturalWidth === 0` で見直している
- `/secretary/*.webp` は `src/proxy.ts` のmatcherが拡張子で外しているので、未ログインでも `/login` へ
  飛ばされない
- 大きさはスマホで幅 `min(230px,62vw)`、PCで260〜300px。**大きくするときはスマホ（393×852）で
...
**#279から既定は「書く」なので、開いただけでは立ち絵の画面は出ない。** 全画面の「話す」を
