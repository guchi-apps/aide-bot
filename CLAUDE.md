...
- **往復のあいだマイクの接続（`getUserMedia`）を掴み続ける対策（#179）は、#197で消した。**
  手放して間を置く（#164）では届かなかったので、**手放す代わりに録音を含む扱いのまま握り
  続ける**形へ切り替えたものだったが、実機では**1往復目しか通らないまま——つまり目的を
  達成しないまま——2往復目以降が `aborted` で終わるようになった**（下記「端末が聞き取りを
  中断する（#197）」）。`src/lib/speech/mic-stream.ts` と声の設定の `holdMic` ごと消してある。
  **設定は既定を変えるだけにしないこと**——localStorageに保存済みの値が読まれ続けるので、
  実機で切り替わらない。**#179の実機での切り分けで分かった次の3つは、いまも有効**
