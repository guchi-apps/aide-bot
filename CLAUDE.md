...
**#279から既定は「書く」なので、開いただけではロボットの画面は出ない。** 音声バー（入力欄の
隣のマイク）で確かめるならそのままでよく、**全画面の「話す」を確かめるなら先に Cookie
`aide-bot-talk-mode=voice` を置く**（ヘッダーの切り替えを押してもよいが、押した回は
`ChatPanel` → `VoicePanel` の作り直しを挟むので、押した直後の1〜2秒は掴む要素が入れ替わる）。
