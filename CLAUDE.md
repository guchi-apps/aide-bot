...
`gpt-6-astra`（最高性能）・`gpt-6-sol`（バランス）・`gpt-6-luna`（高速）。#349でGPT-5.6系から移したが、
GPT-6系に対応していないアカウント向けに#358で5.6系（sol・terra・luna）も選べるようにした（既定にはしない）。サブスクの定額制なので、賢さとサブスクの利用枠
