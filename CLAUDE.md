...
- **実機（#210）では、端末の声なら2往復目以降も聞き取れるようになり、VOICEVOXでは聞き取れない
  ...
  `<audio>` の再生はセッションを「再生」へ切り替え、止めた後も戻るまでに間がある。
  **VOICEVOXの後だけマイクを開くまでの間を長くしてある**（`RESUME_AFTER_AUDIO_ELEMENT_MS`。
  2.5秒。端末の声は400msのまま）。#164が「400msでは足りないことがある」と書いていたのはこの形で、
  経路ごとに待ちを分けるのが要点——端末の声まで同じだけ待たせる理由は無い
