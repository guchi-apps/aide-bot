...
- **接続を取り直すのは利用者が押した流れ（`prime()`）だけ。** 読み上げのあとの自動の開き直しは
  `prime()` を通らないので、いちど手放したら**次に押されるまで保たないまま**になる。裏を返すと、
  **記録に「マイクの接続を保った」が出ないことは「手放している」を意味しない**——
  `holdMicStream()` はすでに保っていればそのまま戻る。往復ごとの有無は
  `マイクを開いた（…・接続あり／なし）` の側で読むこと
