...
- **開発DBのダミー（`scripts/seed-ci-db.mjs` の `BRIEFING_USAGE_MODEL`）はClaudeのまま残してある。**
  `BRIEFING_MODEL` と揃えると従量課金の節が常に空になり、その節の表示が壊れていても画面から
...
- **歯止めは3つ。** 話題からは `NUDGE_INTERVAL_MS`（30分）に1回まで・最後の発言から
  `NUDGE_QUIET_MS`（3分）は積まない・同じ材料は一度だけ（`Notice.shownAt`・`Topic.spokenAt`）。
  前の2つは純粋な関数にしてテストで固定してある（`test/nudge-choice.test.ts`。#265と同じ分け方）
- **用件（お知らせ）は待たせない。** 3分の間合いを見るのは話題だけで、`appendNudge()` は歯止めを
  見ない。「逃すと困る」のは用件の側で、雑談だけが待てる
- **積む印はDBを待つ前に同期で立てる**（#263の `running`・#144の `attempts` と同じ形）。2つのタブの
...
- **Web Pushは増やしていない。** 届くのは画面を開いているときだけで、#79（1日1本・黙れること）・
  ...
  作らない」から設計し直すこと
- **画面の名札は「秘書から」へ差し替える**（`entry-list.tsx` の `SecretaryLabel`）。頼んでいないのに
  現れる発言なので、返答と同じ見た目だと「何に対する返事か」を探すことになる。読み上げソフトへは
  `sr-only` で「話しかけました」まで伝える
...
- **モデルは `HOME_PROFILE_MODEL`（`gpt-5.6-terra`）。** `COMPACT_MODEL` と同じ理由で中位
...
- **画面の中央にいるロボットは `src/components/voice/robot.tsx`、動きは `globals.css` の `.bot` 系**
  （#49）。**いまはWebGLで3Dモデルを描いており（`robot-3d/`。PR #176）、`.bot` 系のCSSが効くのは
  WebGLが使えないときのSVGにだけ**（触れたときの反応と会話の仕草は下記「ロボットに触れる・目で追う」）。待つ・聞く・考える・話すの4状態を1つの値から出し分ける。**部品を動かすときは
...
3Dのロボット（PR #176）に、カーソル・タップへの反応と、会話状態ごとの仕草を足した。
**部位ごとに誰が値を決めるかを固定してある**——強い順に**会話の状態 → タップ反応 → 視線追従**で、
上のものが下のものを上書きする（`robot-3d/model.ts` の `update()`）。会話の状態がいちばん強いのは、
いま聞いているのか考えているのかが読めなくなるのがいちばん困るため。

- **`update()` には1フレームぶんの入力（`RobotDrive`）をすべて渡し、モデル側で時刻も入力も
  覚えない。** 非表示・画面外で描画を止めている間に位相だけが進むと、戻ってきた瞬間に跳ねる
- **`scene.ts` の `wake()` と `resume()` を使い分ける。** `wake()` は `lastTime` を `-Infinity` へ
  戻して即座に1こま描くためのもので、**カーソルが動くたびにこれを呼ぶと毎フレームの経過が0になり、
  目標へ寄せる処理が一度も進まないまま視線が固まる。** 追従とタップ反応は `resume()`（止まって
  いたら再開するだけ）を使う
- **アンテナの発光は明るさを上げるだけでは見えない。** 168pxの表示ではランプが4px程度しかなく、
  `emissiveIntensity` を0.5→1.35にしても、頭のあたりだけを切り出した実測で待機中の揺れと
  見分けが付かなかった（絵の差の平均が0.01。揺れだけで3.1）。ランプの外へはみ出す `Sprite`
  （放射グラデーションの光）を重ねてある。**`Sprite` は `Mesh` ではないので `dispose()` の
  `traverse` では拾えない**——材質とテクスチャは名指しで手放す
- **押された場所の見分けは、名前ではなくグループの祖先で行う**（`Antenna` グループ）。名前の一致に
  すると、部品を1つ足すたびに判定側の文字列も足すことになる。**当たり判定用の透明なメッシュを
  別に置いてある**——three.jsのレイキャストは `visible = false` を見ないため、透明な材質で
  「写らないが当たる」を作る（ランプだけを的にすると指では押せない）
- **スマホの縦スクロールを塞がないための錠は2つ。** `touch-action: pan-y` と、
  「指が12px以上動いた回・0.7秒以上押された回は反応にしない」。**ロボットを押しても
  マイク・送信・読み上げは動かない**（描画側から会話状態を変えないという#176の前提のまま）。
  **#191でページ全体（`html`/`body`）はスクロールしなくなったが、`touch-pan-y`は外していない**
  ——ロボットを囲む中央列（`voice-panel.tsx`）は同じ#191で `overflow-y-auto` を持つように
  なり（hint・notice・errorが重なって縦に収まりきらないときの逃げ場）、ロボットの上を指で
  なぞって**その列を**縦に送れる必要が残っているため
- **キーボードでの押下は `onClick` の `event.detail === 0` で拾う。** `pointerup` と両方で受けると、
  マウスの1クリックが2回反応する
- **動きを減らす設定では、追従も会釈も止めてアンテナの発光だけで応える。** `robot.tsx` 側
  （SVG）と `scene.ts` 側（3D）の**両方**に同じ判定がある。片方だけ緩めない

### タップ反応を増やすときは、選ぶロジックを独立したファイルに置く（#215）

会釈に加えて足上げ・ジャンプを足したとき（体を押されるたびに3種類からランダムに1つを選ぶ）、
その選択ロジック（`pickBodyReaction()`）を `robot-3d/scene.ts`（3D）にも `robot.tsx`（SVG
フォールバック）にも属さない `robot-reaction.ts` という独立したファイルに置いた。

- **`robot.tsx` から `robot-3d/scene.ts` の関数を直接importしてはいけない。** `robot-3d/scene.ts`
  はファイル先頭で `three` をimportしており、たとえ使うのが型だけでなく実行時の関数1つでも、
  `robot.tsx`（SVGフォールバックの実体。WebGL非対応・初期化失敗の端末で表示され続ける）が
  それをimportした時点でthree.js一式がSVGフォールバック側のバンドルにも混ざる。「3Dの初期化に
  失敗した端末でも会話とSVGを継続する」（`Robot` コンポーネントの `catch` 節）という設計が、
  バンドルサイズの面で骨抜きになる
- **`import type` は無罪、実行時のimportが問題。** 既存の `import type { RobotPart } from
  "./robot-3d/scene"` はビルド時に消えるので安全。新しく共有したいのが値（関数・定数）なら、
  three.jsに依存しない別ファイルへ切り出し、3D・SVGフォールバックの両方から静的importする
- 反応どうしは排他（同時に2つ以上が真になることはない）という前提を、3D側の`activeUntil()`・
  SVGフォールバック側のタイマーの両方が置いている。新しい反応を足すときは両方に同じ前提を持たせること

### ロボットをドラッグで回転させる（#201）

スワイプ・マウスドラッグの横方向の動きでロボットをY軸回転させ、指を離した後も
惰性（慣性）で少し回り続けるようにした。3D（`robot-3d/`）とSVGフォールバック
（`robot.tsx`）の両方に対応する。

- **回転は「会話の状態 → タップ反応 → 視線追従」（#180）の外側の軸に持たせた。**
  視線・状態はいずれも`model.ts`の`body.rotation`を合成して決めており、ここへ
  ドラッグ回転を混ぜると3層の優先順位を崩すことになる。そこで`scene.ts`側の
  `root.rotation.y`（`body`を含むグループの外側）へ別枠で足す形にした——
  `model.update()`の計算とは完全に独立していて、互いを上書きしない
- **慣性の計算は`scene.ts`ではなく`robot.tsx`に置く。** `setSpin(radians)`は
  受け取った角度をそのまま描画するだけの薄いAPIで、`robot.tsx`側の
  `requestAnimationFrame`ループが角速度を減衰させながら同じ角度を3D
  （`controller.setSpin()`）とSVGフォールバック（`<svg>`への
  `style.transform = rotateY(...)`）の両方へ渡す。3D・SVGで別々に慣性を
  実装せずに済む
- **SVGフォールバックは`perspective`＋`rotateY()`で疑似3D回転にした。** SVG自体は
  平面の絵だが、親要素に`perspective`を設定してから`rotateY()`を当てると、
  奥行きのある回転に見える。SVGルート要素（`<svg className="bot ...">`）には
  元々transformが付いていない（`.bot`は`display`/`flex`のみ、動きはすべて内側の
  `.bot-body`等に閉じている）ため、インラインstyleと衝突しない
- **「動きを減らす」設定では、ドラッグへの追従は止めず、指を離した後の惰性だけ
  止める。** 追従はユーザーの直接操作の結果であって自動アニメーションではないため
- CDP（`chrome-headless-shell`にリモートデバッグポートを付けて直接WebSocketで
  操作する。Playwright不要）でマウスドラッグをディスパッチし、3D版はスクリーン
  ショットの見た目、SVGフォールバック版は`style.transform`の値の変化で、
  ドラッグへの追従と慣性の両方を実機無しで確認できた

#### 画面の絵で確かめるときに踏むところ

- **切り抜く枠は撮るたびに測り直す。** 会話が進むと文が増えてロボットの位置が下がるため、
  固定した切り抜きでは「姿勢が変わった」と「画面がずれた」を見分けられない（実測で、同じ
  状態遷移の差が16→56へ3倍以上に膨らんだ）
- **等値（バイト一致）の比較は使えない。** 待機中もロボットは揺れているので、2枚が一致することは
  ない。差の**量**で見ること。物差しには「待機中の揺れ」の差（実測で平均2.0前後）を使う。
  ただし「動きを減らす」設定（`Emulation.setEmulatedMedia`）を有効にすると完全に止まる（差が0）
  ので、そのときだけ等値が使える
- PNGを比べる手段はNodeに無いので、**2枚をページへ送り返して `<canvas>` で差を出す**のが早い
- WebGLが使えない端末は、`HTMLCanvasElement.prototype.getContext` が `webgl` で `null` を返すよう
  差し替えれば真似られる（`Page.addScriptToEvaluateOnNewDocument`）

### 描く解像度は端末の画素密度に合わせる（#190）

**`setPixelRatio()` の上限を下げると、そのぶんそのまま粗く見える。** 描いた絵をブラウザが表示の
大きさへ引き伸ばすためで、輪郭・目のリング・顔の陰影が一様にぼやける。#176から上限を1.5に
していたが、DPR 3のiPhoneでは168pxの枠を252²で描いて504²へ2倍に拡大しており、それが「粗い」の
正体だった（#190で実測）。いまは上限3。

- **手元のサブPCでは一度も再現しない。** ヘッドレスChromeは既定でDPR 1なので、
  `Emulation.setDeviceMetricsOverride`（`deviceScaleFactor: 3`）を明示しないと**粗さそのものが
  画面に出ない**。効いているかは `canvas.width`（描いている画素）と `canvas.clientWidth`
  （表示の大きさ）を見比べれば分かる——**同じ値なら等倍、`width` が小さければ引き伸ばしている**
- **上限を上げても増えるのは塗る画素だけ。** 頂点数（52,300三角形）・描画回数（13回）・30fps上限・
  画面外での停止は変わらない。168pxの枠で252²→504²（約4倍）だが、面積そのものが小さい
- **`anisotropy` は実際に材質へ渡すテクスチャに付ける。** `model.ts` には設定値を持つだけで
  どの材質にも渡っていないテクスチャがあり、そちらにだけ付いていたため、体の編み目（128pxを
  14×8回繰り返す）は等方フィルタのままだった。**作った枚数と渡した枚数が合っているかを見ること**
  （three.jsは材質に紐づいて描かれたときに初めてGPUへ送るので、**渡していないテクスチャは
  GPUの負荷ではない**。消す値打ちは可読性であって性能ではない）
- **画素を増やしたぶん `antialias` を落とす、は採らない。** DPR 3まで上げても等倍なので、
  1デバイス画素ぶんの階段はそのまま残る。実測（`prefers-reduced-motion` で静止させ、
  等倍504²を前後で撮った）でRMSEが4.5%、**アンテナの球と支柱の輪郭に目に見える階段**が出た。
  MSAAはタイル方式のモバイルGPUでは比較的安く、粗さを直すのが目的の変更で輪郭を粗くしては
  本末転倒なので残してある
- 見比べには**同じ枠を等倍（DPR 3なら504²）で切り出して並べる**。撮影時に
  `Page.captureScreenshot` の `clip.scale` を足すと `deviceScaleFactor` と掛け算になり、
  168pxの枠が1512²で返る（粗さの比較そのものは崩れないが、等倍ではない）
