 ...
 * 音声で聞くときに足す指示（#27）。
 *
 * 画面で読むのと声で聞くのとでは、読みやすい返答の形がまるで違う。見出しや表は読み上げると
 * 意味を成さず、長い返答は聞き終わるまで戻れない。文字のときの人格は変えず、形だけを変える。
 */
export const VOICE_STYLE_INSTRUCTION = `この返答は音声で読み上げられます。上のルールに加えて、次を守ってください。

- 3文以内・200文字以内を目安にする。続きがあるときは最後に「詳しく話しますか」と一言だけ添える
- 見出し・箇条書き・表・コードブロック・記号の装飾を使わない。読み上げても伝わらない
