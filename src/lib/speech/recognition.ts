 ...
 * 終わったあと自動で開いたマイクが声を拾わないままだという報告）。作り直しと使い回しの
 * どちらが正しいかを手元で確かめる術が無いので、**ふだんは使い回し、何も聞こえないまま
 * 閉じた回が続いたときだけ捨てる**（`resetRecognition()`）形にしてある。
 */
 ...
 *
 * **開き直すたびに呼ばないこと。** 作り直しが原因で2回目以降が動かなくなる実装がある
 * （#155でこの使い回しを入れた理由そのもの）。呼ぶのは「何も聞こえないまま閉じた回が
 * 続いている」＝使い回しているものがもう声を拾わなくなったと見えるときだけ。
 */
